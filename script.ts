...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const triggerBattle = () => {
        // Placeholder battle trigger — switch to battle tab and play a sound cue
        ...
        battleAudio.play().catch(() => {}); // silently ignore autoplay block
        // TODO: actual battle logic here — for now just navigate to battle tab
    ...
    const handleTileInteraction = (x: number, y: number) => {
        ...
        if (definition.resourceType === 'wood') {
            ...
            // walkable tiles (grass, bush, etc.) — reduce uses
    ...
    // Re-render deck tab when switching to it
    document.querySelectorAll('[data-tab]').forEach(btn => {
        btn.addEventListener('click', () => {
            if ((btn as HTMLElement).dataset.tab === 'deck-tab') renderDeckTab();
        });
    });
